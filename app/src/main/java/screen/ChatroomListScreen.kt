...
fun ChatRoomListScreen(roomViewModel: RoomViewModel = viewModel(),
    ...
    Column(modifier = Modifier
        ...
        LazyColumn {
            items(rooms){room ->
                RoomItem(room = room, onoJoinClicked = {onJoinClicked(room)})
            }
        }
        Spacer(modifier = Modifier.height(16.dp))
                ...
                confirmButton = {
                    Row(modifier = Modifier
                        ...
                        Button(
                            onClick = {
                                if (name.isNotBlank()) {
                                    roomViewModel.createRoom(Room(name = name))
                                    showDialog = false
                                }
                            }
                        ) {
...
@Composable
fun RoomItem(room: Room, onoJoinClicked: (Room) -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp),
